---
title: Forms
description: "Zen provides a complete form system built on react-hook-form."
---

import { Box, Button, Checkbox, Column, Example, Form, FormButtons, FormController, FormErrorExample, FormExample, FormField, FormFieldArray, FormFieldArrayExample, FormInputsExample, FormLoadingExample, FormResetButton, FormSubmitButton, FormValidationExample, PasswordField, Radio, RadioGroup, Row, SearchField, Select, Slider, Switch, Text, TextField } from '../../../mdx-components';
...
<PropertiesTable>
  <ResponseField name="defaultValues" type="object">Initial form values (uncontrolled)</ResponseField>
  <ResponseField name="values" type="object">Current form values (controlled)</ResponseField>
  <ResponseField name="onSubmit" type="(data) => void | Promise">Called when form is submitted with valid data</ResponseField>
  <ResponseField name="error" type="ReactNode | Error">Error message to display above the form</ResponseField>
  <ResponseField name="preventSubmit" type="boolean">Prevents form submission on Enter key</ResponseField>
  <ResponseField name="resolver" type="Resolver">Validation resolver for schema validation (Zod, Yup, etc.)</ResponseField>
  <ResponseField name="mode" type="string">Validation mode: "onSubmit" | "onBlur" | "onChange" | "onTouched" | "all"</ResponseField>
</PropertiesTable>

### FormField

<PropertiesTable>
  <ResponseField name="name" type="string">Field name (required). Must match a key in form values.</ResponseField>
  <ResponseField name="label" type="string">Label displayed above the input</ResponseField>
  <ResponseField name="description" type="string">Help text displayed below the input</ResponseField>
  <ResponseField name="rules" type="RegisterOptions">Validation rules (required, min, max, pattern, validate, etc.)</ResponseField>
</PropertiesTable>

### FormFieldArray

<PropertiesTable>
  <ResponseField name="name" type="string">Field array name (required)</ResponseField>
  <ResponseField name="label" type="string">Label displayed above the field array</ResponseField>
  <ResponseField name="description" type="string">Help text displayed below the label</ResponseField>
  <ResponseField name="children" type="(props) => ReactNode">Render function receiving fields, append, remove, and control</ResponseField>
</PropertiesTable>

### FormSubmitButton

<PropertiesTable>
  <ResponseField name="variant" type="ButtonVariant">Button style variant (default: "primary")</ResponseField>
  <ResponseField name="isDisabled" type="boolean">Override automatic disabled state</ResponseField>
  <ResponseField name="isLoading" type="boolean">Override automatic loading state</ResponseField>
</PropertiesTable>

### FormResetButton

<PropertiesTable>
  <ResponseField name="values" type="object">Values to reset the form to (default: original defaultValues)</ResponseField>
</PropertiesTable>
